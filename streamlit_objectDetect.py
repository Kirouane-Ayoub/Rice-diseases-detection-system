...
def draw_boxes(img, bbox, identities=None, categories=None, names=None,offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        #label = "num:"+ str(id)
        label = f"ID : {id} and  its {cat}"
        (w, h), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_DUPLEX, 0.6, 1)
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 1)
        cv2.rectangle(img, (x1, y1 - 20), (x1 + w, y1), color, -1)
        cv2.putText(img, label, (x1, y1 - 5),cv2.FONT_HERSHEY_DUPLEX, 0.6, [255, 255, 255], 1)
        cv2.putText(img,f"Total number : {str(max(count_list))}", (50,50), cv2.FONT_HERSHEY_DUPLEX,1, 175 , 2)
